...
# The script will setup cloud infrastructure and S/4 HANA software
...
INFO=`cat <<EOF
    ...
    -----------------------------------------------
    1     | auto install            | Automatic setup cloud infrastructure and SAP S/4 HANA application
    2     | manull install          | Setup cloud infrastructure and install SAP S/4 HANA application step by step
    3     | Exit                    |
EOF
`
STEP_INFO=`cat <<EOF
    ...
    5     | install S/4 HANA   | Install SAP S/4 HANA software
...
function check_params(){
    ...
    check_para ApplicationVersion '^S/4 HANA (1909|1809|1709)$'
}
...
function install_S4_1709() {
    ...
    info_log "Start to install S/4 HANA 1709"
...
EOF
    ...
    validation || return 1
    info_log "Finished S/4 HANA 1709 single node installation" 
}
...
function install_S4_1809() {
    ...
    wait_HANA_ECS "${HANAPrivateIpAddress}" "${HANAInstanceNumber}" || return 1
    info_log "Start to install S/4 HANA 1809"
    sapinst_path=${QUICKSTART_SAP_EXTRACTION_DIR}"/${TAR_NAME_SW%.*}/"
...
EOF
    cd "${sapinst_path}" && ./sapinst SAPINST_INPUT_PARAMETERS_URL=${SAP_INSTALL_TEMPLATE_PATH} SAPINST_EXECUTE_PRODUCT_ID="${SAPINST_EXECUTE_PRODUCT_ID}" SAPINST_SKIP_DIALOGS=true SAPINST_START_GUISERVER=false  >/dev/null || return 1
    validation || return 1
    info_log "Finished S/4 HANA 1809 single node installation"
}
...
function install_S4_1909() {
    ...
    wait_HANA_ECS "${HANAPrivateIpAddress}" "${HANAInstanceNumber}" || return 1
    info_log "Start to install S/4 HANA"
    sapinst_path=${QUICKSTART_SAP_EXTRACTION_DIR}"/${TAR_NAME_SW%.*}/"
    MediaPath=${QUICKSTART_SAP_DOWNLOAD_DIR}"/"
    HANA_sid=$(echo "$HANASID" |tr '[:upper:]' '[:lower:]')"adm"
...
EOF
    cd "${sapinst_path}" && ./sapinst SAPINST_INPUT_PARAMETERS_URL=${SAP_INSTALL_TEMPLATE_PATH} SAPINST_EXECUTE_PRODUCT_ID="${SAPINST_EXECUTE_PRODUCT_ID}" SAPINST_SKIP_DIALOGS=true SAPINST_START_GUISERVER=false  >/dev/null || return 1
    validation || return 1
    info_log "Finished S/4 HANA single node installation"
}

#Define validation function
#validation
function validation() {
    info_log "Start to check S/4 HANA running status"
    ...
    then
        info_log "S/4 HANA is running"
        return 0
    else
        error_log "S/4 HANA status is unknown"
        return 1
    fi
}
